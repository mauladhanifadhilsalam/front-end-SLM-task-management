...
export default function AdminComments() {
        ...
        const list: AdminComment[] = candidate.map((c: any): AdminComment => ({
            ...
            ? {
                ...
                project: c.ticket.project
                    ? { id: Number(c.ticket.project.id), name: c.ticket.project.name ?? undefined }
                    : null,
                }
    ...
    const handleDelete = async (id: number) => {
        ...
        text: `Are you sure you want to delete comment #${id}${target?.message ? `: "${target.message.slice(0, 60)}..."` : ""}`,
        icon: "warning",
            ...
            <div className="flex flex-1 flex-col">
                ...
                <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <Button
                            size="sm"
                            onClick={() => navigate("/admin/dashboard/comments/create")}
                            className="cursor-pointer"
                    ...
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuCheckboxItem
                                checked={cols.sel}
                                onCheckedChange={(v) => setCols((c) => ({ ...c, sel: !!v }))}
                            >
                                Selection
                            </DropdownMenuCheckboxItem>
                            <DropdownMenuCheckboxItem
                            ...
                            <DropdownMenuCheckboxItem
                                checked={cols.ticket}
                                onCheckedChange={(v) => setCols((c) => ({ ...c, ticket: !!v }))}
                            >
                                Ticket
                            </DropdownMenuCheckboxItem>
                            <DropdownMenuCheckboxItem
                                checked={cols.user}
                                onCheckedChange={(v) => setCols((c) => ({ ...c, user: !!v }))}
                            >
                                User
                            </DropdownMenuCheckboxItem>
                            <DropdownMenuCheckboxItem
                                checked={cols.message}
                                onCheckedChange={(v) => setCols((c) => ({ ...c, message: !!v }))}
                            >
                                Message
                            </DropdownMenuCheckboxItem>
                            <DropdownMenuCheckboxItem
                                checked={cols.created}
                                onCheckedChange={(v) => setCols((c) => ({ ...c, created: !!v }))}
                            >
                                Created
                            </DropdownMenuCheckboxItem>
                            <DropdownMenuCheckboxItem
                    ...
                    <Card className="rounded-md border overflow-x-auto">
                        ...
                        <thead className="bg-muted/50">
                            ...
                            {cols.sel && (
                                ...
                                <input
                                    ...
                                    checked={currentPageAllSelected && pageRows.length > 0}
                                    onChange={toggleSelectAllOnPage}
                                />
                                </th>
                            )}
                            {cols.id && <th className="px-4 py-3 text-sm font-medium">ID</th>}
                            {cols.ticket && <th className="px-4 py-3 text-sm font-medium">Ticket</th>}
                            {cols.user && <th className="px-4 py-3 text-sm font-medium">User</th>}
                            {cols.message && <th className="px-4 py-3 text-sm font-medium">Message</th>}
                            {cols.created && <th className="px-4 py-3 text-sm font-medium">Created</th>}
                        ...
                        <tbody className="divide-y divide-border bg-background">
                            ...
                            pageRows.map((c) => {
                                const uname =
                                c.user?.fullName || c.user?.name || c.user?.email || `User#${c.userId}`;
                                const urole = c.user?.role;
                                const tTitle =
                                c.ticket?.title ||
                                (c.ticket?.project?.name ? `${c.ticket?.project?.name}` : undefined);
                                return (
                                <tr key={c.id} className="align-top">
                                    ...
                                    {cols.id && <td className="px-4 py-3 text-sm">{c.id}</td>}
                                    {cols.ticket && (
                                    <td className="px-4 py-3 text-sm">
                                        <div className="flex items-center gap-2">
                                        <Badge variant="outline">#{c.ticketId}</Badge>
                                        {tTitle && (
                                            <span className="text-muted-foreground">{tTitle}</span>
                                        )}
                                        <Link
                                            to={`/admin/dashboard/tickets/view/${c.ticket?.id ?? c.ticketId}`}
                                            title="View Ticket"
                                    ...
                                    <td className="px-4 py-3 text-sm">
                                        <div className="flex items-center gap-3">
                                            <Link to={`/admin/dashboard/comments/view/${c.id}`}>
                                                    <IconEye className="h-4 w-4 text-muted-foreground hover:text-foreground" />
                                            </Link>
                                            <Link to={`/admin/dashboard/comments/edit/${c.id}`}>
