...
export default function AdminProjectPhasePage() {
  ...
  // 🔥 fungsi delete dipanggil dari AlertDialogAction
  const deletePhase = async (id: number) => {
    ...
    try {
      ...
      toast.success("Phase deleted", {
        description: `Phase "${phase?.name ?? `#${id}`}" has been deleted.`,
      })
    } catch (err: any) {
      setPhases(prev)
  ...
  const colSpan = Object.values(cols).filter(Boolean).length || 8
                        ...
                        {loading ? (
                          ...
                          filtered.map((phase) => (
                            <tr key={phase.id} className="text-center">
                              {cols.id && (
                                <td className="px-4 py-3 text-sm">{phase.id}</td>
                              )}
                              {cols.name && (
